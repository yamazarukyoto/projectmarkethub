...
## 62. 修正計画（2026-01-02 契約前の添付ファイルダウンロード不可問題の修正）

### 62.1 問題の概要
契約前に、クライアントが添付したもの、ワーカーが添付したもの双方が、ダウンロードできない。

### 62.2 原因分析
1. **ワーカーの仕事詳細ページ（`/worker/jobs/[id]`）**：
   - 添付ファイルセクションは表示されている
   - ダウンロードアイコンは表示されているが、ファイル名が空（`att.name`が空または未定義）
   - コードは正しいが、データの問題（`att.name`が空）の可能性

2. **クライアントの案件詳細ページ（`/client/jobs/[id]`）**：
   - ワーカーの応募時の添付ファイル（`proposal.attachments`）が表示されていない
   - 応募一覧カードに添付ファイル表示がない

3. **メッセージページ（`/messages/[roomId]`）**：
   - 案件の添付ファイルや応募時の添付ファイルが表示されていない
   - 契約前の交渉段階で添付ファイルを確認できない

### 62.3 修正内容

#### 1. ワーカーの仕事詳細ページ（`/worker/jobs/[id]/page.tsx`）
- `att.name`が空の場合のフォールバック表示を追加
- URLからファイル名を抽出するロジックを追加

#### 2. クライアントの案件詳細ページ（`/client/jobs/[id]/page.tsx`）
- 応募一覧カードにワーカーの添付ファイル表示を追加

#### 3. メッセージページ（`/messages/[roomId]/page.tsx`）
- サイドパネルに案件の添付ファイルと応募時の添付ファイルを表示するセクションを追加

### 62.4 実装手順
1. `src/app/(main)/worker/jobs/[id]/page.tsx` - ファイル名フォールバック追加
2. `src/app/(main)/client/jobs/[id]/page.tsx` - 応募者の添付ファイル表示追加
3. `src/app/(main)/messages/[roomId]/page.tsx` - 添付ファイルセクション追加
4. デプロイして動作確認

---

## 63. 修正計画（2026-01-02 添付ファイルダウンロード・メッセージスクロール問題の修正）

### 63.1 問題の概要
1. **問題①**: 契約前にクライアントがアップロードしたファイルがダウンロードできない。ファイル名が「ファイル１」と表示される。
2. **問題②**: ワーカーもクライアントも、メッセージが多いとすべて表示されないので、スクロールで全文読めるようにしてほしい。

### 63.2 原因分析

#### 問題①の原因
1. **Firebase StorageのURLでは`download`属性が効かない**: Firebase StorageのURLは直接ダウンロードではなく、ブラウザで開く形式になっている
2. **ファイル名抽出ロジックの問題**: URLからファイル名を抽出する際、エンコードされたパスの解析が正しく行われていない可能性がある

#### 問題②の原因
1. **メッセージ本文のスクロール制限がない**: ChatBoxコンポーネントで、長いメッセージがある場合にメッセージバブル全体が大きくなり、他のメッセージが見えなくなる
2. **メッセージ本文に`max-h`と`overflow-y-auto`が設定されていない**

### 63.3 修正内容

#### 1. 添付ファイルのダウンロード問題（問題①）
- `target="_blank"`でファイルを新しいタブで開く方式は維持（Firebase Storageの制約）
- ファイル名抽出ロジックを改善し、より確実にファイル名を取得できるようにする
- ファイル名が取得できない場合のフォールバック表示を改善

**修正ファイル:**
- `src/app/(main)/client/jobs/[id]/page.tsx`
- `src/app/(main)/worker/jobs/[id]/page.tsx`

#### 2. メッセージのスクロール問題（問題②）
- メッセージ本文に`max-h-40`（最大160px）と`overflow-y-auto`を追加
- 長いメッセージでもスクロールで全文読めるようにする

**修正ファイル:**
- `src/components/features/message/ChatBox.tsx`

### 63.4 実装手順
1. `src/components/features/message/ChatBox.tsx` - メッセージ本文にスクロール機能を追加
2. `src/app/(main)/client/jobs/[id]/page.tsx` - ファイル名抽出ロジックを改善
3. `src/app/(main)/worker/jobs/[id]/page.tsx` - ファイル名抽出ロジックを改善
4. デプロイして動作確認
...
## 66. 修正計画（2026-01-03 オファー送信エラー・契約重複問題の修正）

### 66.1 問題の概要
1. **オファー送信エラー**: クライアントがオファーを送信するとタイムアウトエラーが発生する
2. **契約重複問題**: オファー送信時に同じ案件の契約が2つ作成されてしまう

### 66.2 原因分析

#### オファー送信エラー
- APIコールがタイムアウトしている
- 既存のハイブリッドアプローチ（fetch + XHR）でも解決できていない

#### 契約重複問題
- リトライが並行して実行された場合、最初のリクエストがまだ完了していない間に2番目のリクエストが実行される
- 両方とも「既存契約なし」と判断して新しい契約を作成してしまう
- 現在の重複チェックは`proposalId`でクエリしているが、並行リクエストでは競合状態が発生する

### 66.3 修正内容

#### 1. 契約作成APIの冪等性強化 (`src/app/api/contracts/create/route.ts`)
- `proposalId`をドキュメントIDとして使用し、Firestoreのトランザクションで重複作成を防止
- `setDoc`の`merge: false`オプションで、既存ドキュメントがある場合はエラーを返す

#### 2. クライアント側のリトライロジック改善 (`src/app/(main)/messages/[roomId]/page.tsx`)
- リトライ前に既存契約をチェックする
- 既存契約が見つかった場合は、その契約IDを使用して遷移

### 66.4 実装手順
1. `src/app/api/contracts/create/route.ts` - 冪等性を強化
2. `src/app/(main)/messages/[roomId]/page.tsx` - リトライロジックを改善
3. デプロイして動作確認
...
### 65.1 依頼内容
1. ワーカーもクライアントも表示名は共通にして、なおかつ記載必須にする
2. ワーカーは表示名以外、任意記述にする

### 65.2 現状分析

#### クライアントプロフィール (`/account/client/profile`)
- **表示名フィールド**: なし（会社名・屋号のみ）
- **必須項目**: なし（全て任意）
- **フィールド**: 会社名・屋号、業種、Webサイト、所在地、電話番号、会社紹介

#### ワーカープロフィール (`/account/worker/profile`)
- **表示名フィールド**: あり（必須）
- **必須項目**: 表示名、キャッチコピー、自己紹介、稼働可能時間
- **フィールド**: 表示名、キャッチコピー、自己紹介、スキル、稼働可能時間

### 65.3 修正内容

#### 1. クライアントプロフィールページ (`src/app/(main)/account/client/profile/page.tsx`)
- **表示名フィールドを追加**: 必須項目として追加
- **保存時**: `displayName`フィールドをユーザードキュメントのルートレベルに保存

#### 2. ワーカープロフィールページ (`src/app/(main)/account/worker/profile/page.tsx`)
- **表示名**: 必須のまま維持
- **キャッチコピー**: 必須 → 任意に変更
- **自己紹介**: 必須 → 任意に変更
- **スキル**: 任意のまま維持
...
### 65.4 実装手順
1. `src/app/(main)/account/client/profile/page.tsx` - 表示名フィールドを追加（必須）
2. `src/app/(main)/account/worker/profile/page.tsx` - 表示名以外を任意に変更
3. デプロイして動作確認
...
## 69. 修正計画（2026-01-03 依頼内容確認モーダルの改善）

### 69.1 依頼内容
1. **依頼形式「プロジェクト方式」の文言を削除**: 不要なので削除
2. **「詳細」セクションを追加**: 依頼の詳細内容を表示し、文字数が多い場合はスクロール可能にする
3. **添付ファイルを表示**: 確認モーダルで添付ファイルを確認できるようにする
4. **カテゴリーを日本語表示**: プルダウンで選択した日本語名と同じになるようにする

### 69.2 修正対象ファイル
`src/app/(main)/client/jobs/new/page.tsx`
...
#### 2. 確認モーダルの変更
- 「依頼形式：プロジェクト方式」セクションを削除
- 「詳細」セクションを追加（max-h-32 overflow-y-auto でスクロール対応）
- 「添付ファイル」セクションを追加
- カテゴリー表示を `categoryLabels[pendingJobData.category]` に変更

### 69.4 実装手順
1. `src/app/(main)/client/jobs/new/page.tsx` を修正
2. デプロイして動作確認
