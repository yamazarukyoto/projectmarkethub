...
export default function WorkerContractsPage() {
    ...
    useEffect(() => {
        const fetchContracts = async () => {
            if (user) {
                try {
                    const data = await getContracts(user.uid, 'worker');
                    setContracts(data);
                } catch (error) {
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            <h1 className="text-2xl font-bold text-secondary mb-6">契約管理 (受注案件)</h1>
            ...
            {contracts.length === 0 ? (
                ...
                <div className="grid gap-4">
                    {contracts.map((contract) => (
                        <Link key={contract.id} href={`/worker/contracts/${contract.id}`}>
                                    ...
                                    <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-500 mb-2">
                                                契約日: {contract.createdAt?.toDate().toLocaleDateString() || '不明'}
                                            </p>
